...
///////////////////////////////////////////////////////////////////////
    ...
    public List<Review> bestproduct(Long idSupplier) {
        ...
        for (Review review : reviewList) {
            ...
            if (review.getAvis() == noteReview.EXCELLENT && review.getIdSupplier().equals(idSupplier)) {
                System.out.println(review.getIdReview() + " " + review.getIdSupplier());
...
//    }
    ...
    public void getBestProductsBySupplier() {
        System.out.println("nahawand start");
        List<Review> reviews = reviewRepository.findAll();
        Map<String, Map<Long, Double>> supplierProductRatings = new HashMap<>();

        for (Review review : reviews) {
            if (review.getAvis() == noteReview.EXCELLENT || review.getAvis() == noteReview.SATISFAIT) {
                String supplierId = review.getIdSupplier();
        ...
        Map<String, List<Long>> topProductsBySupplier = new HashMap<>();
        for (Map.Entry<String, Map<Long, Double>> supplierEntry : supplierProductRatings.entrySet()) {
        ...
        for (Map.Entry<String, List<Long>> entry : topProductsBySupplier.entrySet()) {
    ...
    public String findMostOccurringIdClient() {
        Map<String, Long> idClientCountMap = new HashMap<>();

        List<Review> reviews = reviewRepository.findAll();
        for (Review review : reviews) {
            String idClient = review.getIdClient();
            if (idClient != null) {
                String idClientStr = idClient;
                Long count = idClientCountMap.getOrDefault(idClientStr, 0L);
                idClientCountMap.put(idClientStr, count + 1);
            }
        }
